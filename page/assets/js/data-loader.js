...
class DataLoader {
  ...
  async detectAvailableDates() {
    ...
    // 测试最近几天的数据
    const testDates = ["2025-08-28", "2025-08-27", "2025-08-26", "2025-08-25"];
  ...
  getBestImageUrl(imageResolutions) {
    ...
    // 其次选择Full HD
    const fullHdImage = imageResolutions.find(
      (img) => img.resolution === "Full HD"
    );
  ...
  getThumbnailUrl(imageResolutions) {
    ...
    const standardImage = imageResolutions.find(
      (img) => img.resolution === "Standard"
    );
  ...
  async loadAllData(progressCallback = null) {
    ...
    if (
      ...
      window.WALLPAPER_DATA_INDEX.availableData
    ) {
      // 使用数据索引中的具体文件信息
      console.log("📊 使用数据索引构建加载列表...");
      Object.entries(window.WALLPAPER_DATA_INDEX.availableData).forEach(
        ([country, dates]) => {
          dates.forEach((date) => {
    ...
    if (window.WALLPAPER_DATA_INDEX) {
      console.log(
        `📊 数据索引信息: ${window.WALLPAPER_DATA_INDEX.totalFiles} 个文件`
      );
    }
    ...
    for (const fileInfo of filesToLoad) {
      const promise = this.loadWallpaperData(
        fileInfo.country,
        fileInfo.date
      ).then((data) => {
        loaded++;
    ...
    this.dates = [...new Set(this.wallpapers.map((w) => w.date))]
      .sort()
      .reverse();
  ...
  getCountryStats() {
    ...
    Object.keys(countryInfo).forEach((country) => {
      const countryWallpapers = this.wallpapers.filter(
        (w) => w.country === country
      );
      stats[country] = {
        ...countryInfo[country],
        ...
        dates: [...new Set(countryWallpapers.map((w) => w.date))]
          .sort()
          .reverse(),
      };
    });
